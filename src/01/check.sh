...
check () {
  last_symb=$(echo "$DIR" | tail -c 2)
  if [[ ! -d $DIR ]] || [[ last_symb == "/" ]] ; then
    echo -e "${RED}ERROR: Directory $DIR doesn't exist.${NC}"
    exit 1
  elif [[ ! $NUM_DIR =~ ^[1-9][0-9]*$ ]] ; then
    echo -e "${RED}ERROR: The second argument should be a number.${NC}"
    exit 1
  elif [[ ! $DIR_NAME =~ ^[a-zA-Z]{1,7}$ ]] ; then
...
and extension (no more than 7 characters for the name, no more than 3 characters for the extension).${NC}"
    exit 1
  elif [[ ! $FILE_SIZE =~ ^[1-9][0-9]?[0]?[Kk]+[Bb]+$ ]] ; then
    echo -e "${RED}ERROR: The forth argument should be a number..${NC}"
    exit 1
  else 
    FILE_NAME=$(echo $FILE_FULL | awk -F "." '{print $1}')
    FILE_EXT=$(echo $FILE_FULL | awk -F "." '{print $2}')
    FILE_SIZE=$(echo $FILE_SIZE | awk -F '[Kk]' '{print $1}')
  fi
}
